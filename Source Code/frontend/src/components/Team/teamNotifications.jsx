...
function TeamNotifications() {
  ...
  const fetchData = async () => {
    try {
      const { data: response } = await axios.get(
        `https://${REACT_APP_IP}:${REACT_APP_BACKEND_PORT}/teamnotify/${email}`
      );
  ...
  const Notifications = () => {
    if (len > 0) {
      // console.log("len", len);
      return notifications.map((notification, index, arr) => {
        return (
          <>
            ...
            {index === arr.length - 1 ? (
              <Dropdown.Item className="text-center card-title">
                <Link
                  to={`/AllteamNotifications/`}
                  className="card-title notifylink"
                >
                  المزيد من الإشعارات
                </Link>
              </Dropdown.Item>
  ...
  function handleUpdate(id) {
    ...
    // console.log(id);
    axios
      .post(`https://${REACT_APP_IP}:${REACT_APP_BACKEND_PORT}/updatenotify/`, uploadData)
      .then((res) => {
    ...
    if (len > 0) {
      return (
        <span className="alert">
          <span
            className="badge"
            style={{ background: "#eec515", borderRadius: "50%" }}
          >
            {" "}
            {len}
          </span>
