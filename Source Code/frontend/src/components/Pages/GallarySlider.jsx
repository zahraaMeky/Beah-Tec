...
function GallarySlider() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const { data: response } = await axios.get(
              `https://www.beah-tec.com:80/workshops/`
            );
      ...
      const ImagesGallaryActive = Images.slice(0,1).map((img, i) => {
        return (
            <img className="img-thumbnail" style={{objectFit:'cover',height:"250px", width:"100%"}} src={"https://www.beah-tec.com:80/media/"+img} alt="" loading="lazy" />
        );
      });
