...
function TeamVote() {
  ...
  const MsgAlert = () => {
    Swal.fire(`لقد انتهت فترة التصويت`)
  };
 ...
 const validatePhone = () => {
  ...
  // var isNumber = /^\d+\.\d+$/.test(pho);
  var isNumber = digits_only(pho);
  ...
  const validateInputPhone = (phone_text) => {
    var pho = phone_text.replace(/\s/g, "");
    // console.log("pho: " + pho);
    setphone(pho);
...
const minteam = minTeam.map((team, i) => {
  ...
  const alertVoteFinish = () =>  {
    return (
      <div className="row d-flex justify-content-center">
          <Toast style={{padding:' 0.375rem 0.75rem',width:'95%',textAlign:'Center'}}className="mb-3 alert alert-danger" onClose={() => setphoneExit(false)} show={phoneExit} delay={3000} autohide>
            <Toast.Body>لقد انتهت فترة التصويت</Toast.Body>
          </Toast>
      </div>
    );
  }
    ...
    return (
      <div className="row d-flex justify-content-center">
          <Toast style={{padding:' 0.375rem 0.75rem',width:'95%',textAlign:'Center'}}className="mb-3 alert alert-danger" onClose={() => setphoneWrong(false)} show={phoneWrong} delay={3000} autohide>
            <Toast.Body>خطأ تقني .. يرجى المحاولة لاحقا </Toast.Body>
          </Toast>
      </div>
    );
  }
          ...
          <Modal.Body>
            <form onSubmit={handleSubmit}>
              <div className="input-group mb-3">
                <input
                  onChange={(evt) => {
                    const isValid = validateInputPhone(evt.target.value);
                    setDisable(!isValid);
